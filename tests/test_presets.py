...
def test_selected_preset_keeps_factory_default_dropdown_edit_by_key(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    params: dict[str, str] = {}
    monkeypatch.setattr("moops.group.mo.running_in_notebook", lambda: True)
    ...
    choice._on_change(1)  # type: ignore[attr-defined]
    ...
    assert recreated_choice.value == 10
...
def test_command_box_updates_variant_help_after_branch_edit() -> None:
    g = Group(cli_args=["script.py"])
    mode = g.dropdown(
        ["car", "train"],
        value="car",
    ...
    assert iface.apply_cli_args("script.py --mode train --travel-train-tickets 5") == ()
...
def test_variant_help_follows_live_selector_change() -> None:
    g = Group(cli_args=["script.py"])
    mode = g.dropdown(
        ["car", "train"],
        value="car",
